...
ScavTrap::ScavTrap(const std::string& name): ClapTrap(name) {
	std::cout << "ScavTrap " << this->name_ << " Default Constructor(with name) called" << std::endl;
	this->hit_point_ = init_hp;
	this->energy_point_ = init_ep;
	this->attack_damage_ = init_ad;
}
...
ScavTrap::ScavTrap(const ScavTrap &src) {
	std::cout << "ScavTrap " << src.name_ << " Copy constructor called" << std::endl;
...
ScavTrap::~ScavTrap() {
	std::cout << "ScavTrap " << this->name_ << "'s Destructor called" << std::endl;
}
