...
BOOST_AUTO_TEST_SUITE(TestSuitePmergeMe)
	...
	BOOST_AUTO_TEST_CASE(TestValidInput) {
		PmergeMe pm;
		const char* args[] = {"./PmergeMe", "3", "1", "4", "5", NULL};
		std::vector<int> vec;

		// argsをstd::vectorに変換する関数を呼び出し
		try {
			pm.inputToContainer(5, const_cast<char**>(args), vec);
	...
	BOOST_AUTO_TEST_CASE(TestInvalidInputWithDuplicates) {
		PmergeMe pm;
		const char* args[] = {"./PmergeMe", "3", "1", "4", "1", "5", NULL};
		std::vector<int> vec;
	...
	BOOST_AUTO_TEST_CASE(TestInvalidInput) {
		PmergeMe pm;
		const char* args[] = {"./PmergeMe", "1", "a", "2", NULL};
		std::vector<int> vec;
		BOOST_CHECK_THROW(pm.inputToContainer(4, const_cast<char **>(args), vec), std::invalid_argument);
	}
	...
	BOOST_AUTO_TEST_CASE(TestMergeInsertionSortVector) {
		...
		BOOST_CHECK_EQUAL_COLLECTIONS(pm.getVec().begin(), pm.getVec().end(), expected.begin(), expected.end());
	}
	...
	BOOST_AUTO_TEST_CASE(TestMergeInsertionSortList) {
		...
		BOOST_CHECK_EQUAL_COLLECTIONS(pm.getLst().begin(), pm.getLst().end(), expected.begin(), expected.end());
	}
